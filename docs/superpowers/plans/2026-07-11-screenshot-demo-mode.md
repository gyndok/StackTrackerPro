...
>
> **Review fix (same day):** the first 06-chat capture actually showed the TOP of the thread (not the big-pot prompt) — `ChatThreadView` only scrolled on `.onChange(of: messages.count)`, which never fires when all messages exist before first render. Fixed with a DEBUG-only `.onAppear` scroll-to-last (guarded on `DemoData.isActive`) in `ChatThreadView.swift`; also added an EXIT `trap` to `make-screenshots.sh` so the status-bar override is cleared even if the script dies mid-run. Recaptured the full set; 06-chat now shows the "I have 168k" → "Big pot — +64,000 at Level 11. Log it? Just tell me your cards." pair; 01/03/04 spot-checked, no regressions. Debug + Release builds clean after the view change.
